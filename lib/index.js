...
const resources = module.exports = {
    ...
    hooks: {
    	'page:before': function (page) {
            ...
            case 'local':
                ...
                break;
            case 'github':
                prefix = 'https://raw.githubusercontent.com/' + defaultOption['repo'] + '/' + defaultOption['branch'] + '/{{ file.path | currentPath() }}';
                break;
            default:
                break;
            }

            page.content = page.content.replace(/\.\/([\S]+)\.png/g, prefix + '$1.png');
            page.content = page.content.replace(/\.\/([\S]+)\.gif/g, prefix + '$1.gif');
            page.content = page.content.replace(/\.\/([\S]+)\.jpg/g, prefix + '$1.jpg');
            page.content = page.content.replace(/\.\/([\S]+)\.jpeg/g, prefix + '$1.jpeg');
    ...
    /** Map of new filters */
    filters: {
        currentPath: function (d) {
            return d.match(/[^\\\/]*[\\\/]+/g) ? d.match(/[^\\\/]*[\\\/]+/g).join('') : '';;
        }
    },
